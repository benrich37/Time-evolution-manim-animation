...
class vectormath(Scene):
    def construct(self):
        ...
        # testthing = organizer()
        # ourthing = testthing.give_thing()
        # ourthing.add_updater(
        #     lambda o: o.shift(UP*np.cos(self.time))
        # )

        # testthing = organizer()
        # ourthing = testthing.give_thing()
        # ourthing.add_updater(
        #     lambda o: o.shift(UP * testthing.thelist[0].get_value())
        # )
        ...
        self.add(*testthing.objectlist)
        auto_play()
        self.play(
            testthing.trackerlist[0].animate.set_value(4)
        )
        self.play(
            testthing.trackerlist[0].animate.set_value(-2)
        )
        self.play(
            testthing.trackerlist[1].animate.set_value(8)
        )
...
class organizer(Scene):
    ...
    def __init__(self):
        # self.thing = Tex("Test").add_updater(
        #     lambda t: t.shift(LEFT/1000)
        # )
        self.thing = Tex("Test")
        self.trackerlist = [None]*self.n
        self.objectlist = [None]*self.n
    ...
    def assign_updater(self,obj,i):
        obj.add_updater(
            lambda o: o.set_x(self.trackerlist[i].get_value(),UP)
        )

    def make_list(self):
        for j in np.arange(self.n):
            self.trackerlist[j] = self.make_tracker()

    def fill_list(self):
        for i in np.arange(self.n):
            self.objectlist[i] = self.make_square()
            self.assign_updater(self.objectlist[i],i)
